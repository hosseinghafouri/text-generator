...
let btnGenerateElem = $.querySelector("#btn-generate");
let result = $.querySelector("#result");
// let btnCopyElem = $.querySelector("#copy");

btnGenerateElem.addEventListener('click', (event)=>{
    ...
    if (inputElemValue < 0 || inputElemValue > 9 || inputElemValue === "" || isNaN(inputElemValue)) {
        result.innerHTML = dummyText[randomTextIndex];
    }else {
        let slicedArray = dummyText.slice(0, inputElemValue);
